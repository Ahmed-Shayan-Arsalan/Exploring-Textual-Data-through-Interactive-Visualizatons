...
def analyze_sentiment_words(text):
    ...
    for word, score in word_sentiment:
        if score < -0.5 and word not in sentiment_words["Angry"]:
            sentiment_words["Angry"].add(word)
        elif 0.1 < score < 0.5 and word not in sentiment_words["Anticipation"]:
            sentiment_words["Anticipation"].add(word)
        elif score < -0.2 and word not in sentiment_words["Fear"]:
            sentiment_words["Fear"].add(word)
        elif score < 0 and word not in sentiment_words["Sad"]:
            sentiment_words["Sad"].add(word)
        elif score > 0 and word not in sentiment_words["Happy"]:
            sentiment_words["Happy"].add(word)

    # Convert sets to lists for JSON serialization
    sentiment_words = {sentiment: [{"word": word, "intensity": sia.polarity_scores(word)['compound']} for word in words] for sentiment, words in sentiment_words.items()}
